...
router.get('/video/:id', async (req, res) => {
  ...
  try {
    ...
    const r = await vimeo('GET', `/videos/${req.params.id}?fields=${encodeURIComponent(fields)}`);
...
router.put('/video/:id/tags/:tag', async (req, res) => {
  if (!requireToken(res)) return;
  try {
    const r = await vimeo('PUT', `/videos/${req.params.id}/tags/${encodeURIComponent(req.params.tag)}`);
...
router.delete('/video/:id/tags/:tag', async (req, res) => {
  if (!requireToken(res)) return;
  try {
    const r = await vimeo('DELETE', `/videos/${req.params.id}/tags/${encodeURIComponent(req.params.tag)}`);
    res.status(r.status).end();
  } catch (e) { handleVimeoError(res, e); }
});
...
router.post('/video/:id/pictures', async (req, res) => {
  if (!requireToken(res)) return;
  try {
    const r = await vimeo('POST', `/videos/${req.params.id}/pictures`);
    res.status(r.status).json(await r.json());
  } catch (e) { handleVimeoError(res, e); }
});
...
router.get('/video/:id/transcribe', async (req, res) => {
  if (!requireToken(res)) return;
  try {
    const r = await vimeo('GET', `/videos/${req.params.id}/ai/transcribe`);
...
router.get('/video/:id/transcript/:texttrackId', async (req, res) => {
  ...
  try {
    ...
    const r = await vimeo('GET', `/videos/${req.params.id}/transcripts/${req.params.texttrackId}?fields=${encodeURIComponent(fields)}`);
    res.status(r.status).json(await r.json());
  } catch (e) { handleVimeoError(res, e); }
});
